...
3. Execute the following command to import the secret into the `terraform` state:
    ```bash
    $ terraform import "bitwarden-secrets_secret.secret" "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"

    bitwarden-secrets_secret.secret: Import prepared!
    Prepared bitwarden-secrets_secret for import
    bitwarden-secrets_secret.secret: Refreshing state... [id=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx]
...
4. Execute `terraform show` in order to see the imported information. The most important one for the next step is `key`:
   ...
   $ terraform show
    ...
    # bitwarden-secrets_secret.secret:
    resource "bitwarden-secrets_secret" "secret" {
